...
edata = log2(edata + 1)
edata_centered = edata - rowMeans(edata) #remove rowmeans otherwise everything centered on it
svd1 = svd(edata_centered) #singular value decomposition
names(svd1) # d: variation across   v: variation across genes u: variation across samples
```
...
```{r}
par(mfrow=c(1,2));plot(svd1$v[,1],col=2,ylab="1st PC");plot(svd1$v[,2],col=2,ylab="2nd PC")
```
