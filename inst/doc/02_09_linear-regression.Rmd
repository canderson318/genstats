...
```{r}
colramp = colorRampPalette(1:5)(17)
lm9 = lm(edata[2,] ~ pdata$age)
plot(lm9$residuals,col=colramp[as.numeric(pdata$tissue.type)]);legend("topright", legend = unique(pdata$tissue.type),
       col = colramp[1:length(unique(pdata$tissue.type))], pch = 19, title = "Tissue Type")
```
